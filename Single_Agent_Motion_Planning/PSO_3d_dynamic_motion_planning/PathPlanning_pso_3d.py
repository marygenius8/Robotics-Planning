...
def cal_collision_penalty(obs, Px, Py, Pz):
    """
...
def fitness(X, args):
    ...
    w_g = 1.0   # path goal penalty weight
    w_c = 10.0
    w_s = 0.1
    w_f = 0.1

    nPop, nVar = X.shape
    nPts = nVar // 3            # Number of (internal) breakpoints

    # Coordinates of the breakpoints (start + internal + goal)
    x = np.block([Xs, X[:, :nPts], Xg])
    y = np.block([Ys, X[:, nPts:2*nPts], Yg])
    z = np.block([Zs, X[:, 2*nPts:], Zg])
    ...
    CSz = interp1d(t, z, axis=1, kind=f_interp, assume_sorted=True)
    ...
    Uz = np.diff(Vz, axis=1) / time_step
    ...
    goal = np.hstack((args[1][0], args[1][1], args[1][2]))
    goal_penalty = np.linalg.norm(current_position - goal, axis=1)

    # calculate Collision penalty value
    collision_penalty, count = cal_collision_penalty(obs, Px, Py, Pz)
    ...
    fitness = L * (1.0 + collision_penalty) + w_g * goal_penalty + w_s * (smooth_penalty + jerk_penalty) + w_f * (acceleration_xy_penalty + velocity_xy_penalty + acceleration_xyz_penalty + velocity_xyz_penalty+acceleration_z_penalty+velocity_z_penalty)
    ...
    if (len(args) == 6):
        args[5] = [L, count, Px, Py, Pz]    # if this args can be returned?
...
class PathPlanning:
    ...
    def add_ball(self, ball_idx, x=0.0, y=0.0, z=0.0, r=1.0, Kv=100.0):
        """
        ...
        data = (x, y, z, r, Kv)
        if ball_idx < len(self.obs['Ball']):
            self.obs['Ball'][ball_idx] = data
        else:
            self.obs['Ball'].extend([None]*(ball_idx-len(self.obs['Ball'])+1))
            self.obs['Ball'][ball_idx] = data
    ...
    def optimize(self, nPts=3, ns=100, nPop=40, epochs=500, K=0, phi=2.05,
        ...
        args = [(Xs, Ys, Zs), (Xg, Yg, Zg),  self.obs, ns, f_interp]
        ...
        LB = np.zeros(nVar)
        UB = np.zeros(nVar)
        LB[:nPts] = self.limits[0]          # x lower bound
        UB[:nPts] = self.limits[1]          # x upper bound
        LB[nPts:2*nPts] = self.limits[2]    # y lower bound
        UB[nPts:2*nPts] = self.limits[3]    # y upper bound
        LB[2*nPts:] = self.limits[4]        # z lower bound
        UB[2*nPts:] = self.limits[5]        # z upper bound
        ...
        L, count, Px, Py, Pz = args[5]
        self.sol = (X, L[0], count, Px, Py, Pz)
