    ...
    while len(open_list) > 0:
        ...
        for dir in range(4):
            ...
            if child_loc[0] < 0 or child_loc[0] >= len(my_map) or child_loc[1] < 0 or child_loc[1] >= len(my_map[0]):
               continue
            # check if the child_loc (possible new child location) is not legitimate, i.e. colliding with obstacle in the map
            ...
            if child_loc not in closed_list:
                if child_loc in open_set:
                    # if child_loc corresponding cost 'child_cost' is less than 'existing_node['cost']', substitute the existed node's cost
                    existing_node = open_set[child_loc]
                    if existing_node['cost'] > child_cost:
                        open_set[child_loc]['cost'] = child_cost
                        heapq.heappush(open_list, (child_cost, child_loc, child))
                else:
                    open_set[child_loc] = child
...
# compare the total cost btw nodes
def compare_nodes(n1, n2):
    """Return true if the total cost of n1 is better than n2."""
...
def space_time_a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    """ my_map      - binary obstacle map
        ...
        constraints - constraints defining where agent should or cannot go at each timestep
    """
    ...
    open_list = []  # priority queue for states search, implement in a heap
    closed_list = dict()    # Initialize the dictionary of explored states (visited nodes)
    ...
    while len(open_list) > 0:
        ...
        for dir in range(5):
            ...
            # if the exploring child state has a static obstacle or is constrained, i.e. collision resolution generated constraints by high-level
            ...
            # is the time can be removed to ensure no space grid can be visited more than once in 3d to ensure a path?
            ...
            if child['loc'] in opt_open_set_loc:
                ...
                # if child['g_val'] < min(nparray_open_set_cost[existing_node_idxes]):
...
def space_time_hybrid_a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    """ my_map      - 3d trajectory map
        start_loc   - start position
        goal_loc    - goal position
        agent       - the agent that is being re-planned
